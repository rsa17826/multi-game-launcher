...
after the launcher is opened it may look empty if you don't have any launchers loaded to load them jst put the launchers .py file in the same dir as the `launcher.(cmd/sh)` then rerun `launcher.(cmd/sh)` or `nix run` and the games launcher should be visible
